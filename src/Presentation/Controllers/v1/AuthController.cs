...
[Authorize]
[Route("/api/v1/auth")]
public sealed class AuthController(IMediator mediator, IMapper mapper, ICurrentUserAccessor currentUserAccessor, IDateTimeProvider dateTimeProvider) : ApiController
{
    /// <summary>
    ...
    public async Task<ActionResult<UserDto>> Register([FromBody] UserRegisterCommand command, CancellationToken ct)
    {
        ...
        var token = Jwt.GenerateToken(Jwt.GetUserClaims(user), TimeSpan.FromDays(1), dateTimeProvider);
